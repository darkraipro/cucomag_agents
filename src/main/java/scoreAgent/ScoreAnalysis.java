...
public class ScoreAnalysis {
	...
	public ScoreAnalysis() throws TupleSpaceException{
		connectToSQLSpace();
		registerCallback();
	}
	...
	private void connectToSQLSpace() throws TupleSpaceException {
		...
		try(Scanner scanner = new Scanner(new File("src/main/java/ressources/Connection.txt"))){
			...
			System.out.println("ScoreAgent connected to TupleSpace.");
		}catch(IOException e){
			e.printStackTrace();
		}
	}
	...
	private void registerCallback() throws TupleSpaceException {
		Tuple template = new Tuple(new Field[] {
		...
		Callback callback = new ScoreCallback(tupleSpace);
		
		tupleSpace.eventRegister(Callback.Command.WRITE, template, callback, true, tupleSpace.getSpaces()[0]);
		System.out.println("Score-EventRegister succesful.");
	}
}
