...
public final class StepsAnalysis {

	private TupleSpace tupleSpace;
	private String[] storeConnectionInfo = new String[3];

	public StepsAnalysis() throws NumberFormatException, TupleSpaceException {
		connectToSQLSpace();
		registerCallback();
	...
	private void registerCallback() throws TupleSpaceException {
		...
		Callback callback = new StepsCallback(tupleSpace);
		tupleSpace.eventRegister(Callback.Command.WRITE, template, callback, true, tupleSpace.getSpaces()[0]);
		System.out.println("StepsEventRegister successful.");
	}
}
